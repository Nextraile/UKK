<x-app-layout>
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Status message --}}
            @if (session('status'))
                <div class="mb-6 rounded-md bg-success-50 p-4 text-sm font-medium text-success-600 dark:bg-success-900/30 dark:text-success-400">
                    {{ session('status') }}
                </div>
            @endif
                ...
                <div class="px-4 py-6 sm:px-8 flex flex-col items-center sm:flex-row sm:items-center gap-6">
                    @if ($user->avatar_path)
                        <img src="{{ asset('storage/' . $user->avatar_path) }}"
                             alt="Avatar"
                             class="h-24 w-24 rounded-full object-cover border-4 border-primary-100 dark:border-border-dark" />
                    @else
                        <div class="h-24 w-24 rounded-full bg-primary-100 dark:bg-primary-900/40 flex items-center justify-center border-4 border-primary-50 dark:border-border-dark">
                    ...
                    <div class="text-center sm:text-left">
                        ...
                        <span class="inline-flex items-center mt-2 px-3 py-1 rounded-full text-xs font-semibold
                            @if ($user->isSuperAdmin()) bg-secondary-100 text-secondary-800 dark:bg-secondary-900/40 dark:text-secondary-300
                            @elseif ($user->isAdmin()) bg-primary-100 text-primary-800 dark:bg-primary-900/40 dark:text-primary-300
                            @else bg-success-100 text-success-800 dark:bg-success-900/40 dark:text-success-300
                            @endif">
                    ...
                    <div class="px-4 sm:px-8 py-4 flex items-center justify-between">
                        ...
                        @else
                            ...
                            <a href="{{ route('verification.notice') }}"
                               class="inline-flex items-center ml-2 px-3 py-1.5 text-xs font-semibold rounded-md bg-primary-600 text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                Verifikasi Email
                            </a>
                        @endif
                    </div>
