...
<article class="group bg-white dark:bg-surface-raised-dark rounded-xl shadow-sm hover:shadow-md transition-all overflow-hidden border border-gray-100 dark:border-border-dark">
  <a href="{{ route('marketplace.show', $kost->slug) }}" class="block">
    ...
    <div class="aspect-video bg-gray-200 dark:bg-surface-muted-dark overflow-hidden relative">
      ...
      @if($kost->status === 'active')
        <span class="absolute top-3 left-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-success-100 text-success-800 shadow-sm">
    ...
    <div class="p-5">
      ...
      <div class="mt-4 flex items-baseline justify-between">
        ...
        @if($kost->average_rating)
          <div class="flex items-center text-sm">
            <svg class="w-5 h-5 text-warning-500 fill-current" viewBox="0 0 20 20" aria-hidden="true">
      ...
      {{-- Optional: Facility Tags --}}
      @if($kost->facilities && count($kost->facilities) > 0)
        <div class="mt-3 flex flex-wrap gap-2">
          @foreach(array_slice($kost->facilities, 0, 3) as $facility)
            <span class="px-2 py-1 bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 text-xs font-medium rounded-full">
              {{ $facility }}
            </span>
          @endforeach
          @if(count($kost->facilities) > 3)
            <span class="px-2 py-1 bg-gray-100 dark:bg-surface-muted-dark text-gray-600 dark:text-text-muted-dark text-xs font-medium rounded-full">
              +{{ count($kost->facilities) - 3 }} lainnya
            </span>
          @endif
        </div>
      @endif
